...
本页列出 `agent-platform` 当前实现中已暴露、但不属于 AGW UI 核心对话协议的扩展接口。核心接入优先阅读 [HTTP API](10-协议数据-HTTP核心运行接口.md)、[SSE 事件模型](13-协议数据-SSE传输与请求运行事件.md) 和 [WebSocket 协议](15-协议数据-WebSocket传输协议.md)。

## 1. 使用约定

- 除特别说明外，HTTP 返回 `ApiResponse<T>`。
- 对应 WS route 已注册时，`request.type` 复用同名 HTTP 路径，成功返回 `response` 帧。
- WS 注册表以 [WebSocket 协议](15-协议数据-WebSocket传输协议.md) 的当前 request 映射为准；HTTP 存在不代表 WS 也注册。
...
`/api/query`、`/api/attach`、`/api/submit`、`/api/steer`、`/api/interrupt` 属于核心交互入口，详见 [HTTP API](10-协议数据-HTTP核心运行接口.md) 与 [WebSocket 协议](15-协议数据-WebSocket传输协议.md)。
